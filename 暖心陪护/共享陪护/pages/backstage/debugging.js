...
Page({
    ...
    wx.request({
      ...
      success: function (res) {
        ...
        if (res.data.errorCode == "0") {
          _this.setData({
            device_Ids: res.data.body.macAddr
          })
    ...
    wx.startBluetoothDevicesDiscovery({
      ...
      success(res) {
        wx.onBluetoothDeviceFound(res => {
          ...
          let data = _this.arrayBufferToHexString(advertisData);//二进制转字符串
            console.log("这个是搜索到的mac数据"+data)
            console.log("这个是mac" + device_Ids.replace(/:/g, ""))
            let mac_id = device_Ids.replace(/:/g, "")
            console.log(!!data)
  ...
  //7.0先加密，后开锁
  doKaiSuo: function (data_datas) {
    let _this = this;
    console.log("开锁指令" + data_datas)
    wx.request({
      url: "https://xcx.zhiyunw.com/zhiyunpeihu/service/getEncrypt",
      data: {
        "hexString": data_datas
      },
      dataType: "json",
      header: {
        'content-type': 'application/json'
  ...
  getStatus: function () {
    ...
    wx.writeBLECharacteristicValue({
      ...
      success: function (res) {
        console.log('获取状态成功')
        // _this.getdatapop(get_Data)  //手动执行下一步
      }
    })
  ...
  closeLine: function () {
    let _this = this;
    let device_Ids = _this.data.device_Ids
    console.log("进入断开链接BLEConnection" + device_Ids)
