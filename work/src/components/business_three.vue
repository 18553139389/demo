			...
			<div class="msg">
				<ul class="list">
					<li>
						<span>公司名称</span>
						<div class="choice">
							<input type="text" placeholder="请输入" v-model="company" />
						</div>
					</li>
					<li>
						<span>法人</span>
						<div class="choice">
							<input type="text" placeholder="请输入" v-model="guider" />
						</div>
					</li>
					<li>
						<span>公司地址</span>
		...
		methods: {
			...
			init() {
				if(localStorage.getItem('param')){
					...
					this.company = datas.applyCompanyName 
					this.guider = datas.compayManager 
					this.address = datas.companyAddress 
					this.phone = datas.compayPhone 
					this.img1 = datas.businessLicense 
					this.img2 = datas.introductionLetter 
				}
			},
			//base64转file
			...
			submit() {
				var self = this
				if(this.company == '' || this.guider == '' || this.address == '' || this.phone == '' || this.img1 == '' || this.img2 == ''){
					Toast('内容不能为空')
					return
				}
				this.loading = true
				let datas = JSON.parse(localStorage.getItem('param'))
				datas.uid = this.uid
				datas.applyCompanyName = this.company
				datas.compayManager = this.guider
				datas.companyAddress = this.address
				datas.compayPhone = this.phone
				datas.businessLicense = this.img1
				datas.introductionLetter = this.img2
				datas.car_pic = datas.car_pic.join(',')
				datas.idcard = datas.idcard.join(',')
				console.log(datas,'全部数据')
