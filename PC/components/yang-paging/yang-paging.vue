<template>
	<view>
		<view style="width: 100%;">
			<view class="fy">
				<span v-if="first" @click="init(1)" :style="[customStyle]">
					首页
				</span>
				<span v-if="arrow" :style="[customStyle]" @tap="upper">
					<slot name="arrow-left">上页</slot>
				</span> <span v-for="(item,index) in page" :key="index" @click="init(item)" :style="{background:HN_back(index),color:HN_color(index),'border-radius':`${radius}%`}">
					{{item}}
				</span>
				<span v-if="arrow" :style="[customStyle]" @tap="next">
					<slot name="arrow-right">下页</slot>
				</span>
				<span v-if="first" @click="init(zpage)" :style="[customStyle]">
					尾页
				</span>
			</view>
