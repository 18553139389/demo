<template>
	<view name='KXDateTime'>
		<uni-popup ref="popup" type="bottom">
			...
			<picker-view v-if="visible == 0" :indicator-style="indicatorStyle" :value="value" @change="bindChange">
				<picker-view-column>
			...
			<picker-view v-if="visible == 1" :indicator-style="indicatorStyle" :value="value" @change="bindChange">
				<picker-view-column>
					<view class="item" v-for="(item,index) in years" :key="index">{{item}}年</view>
				</picker-view-column>
				<picker-view-column>
	...
	export default {
		...
		methods: {
			...
			ok() {
				...
				let data = null
				if(this.visible == 1) {
					data = this.year + '-' + month + '-' + day
				} else {
					data = this.year + '-' + month + '-' + day + ' ' + hour + ':' + min
				}
