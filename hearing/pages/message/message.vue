	...
	<view class="contain">
		...
		<view class="wrapper">
			...
			@emptyclick="emptyClick" @topclick="topClick">
				<uni-swipe-action>
					<uni-swipe-action-item style="margin-bottom: 24rpx;background: #f6f6f6;" v-for="(item,index) in list" :options="options" :key="index" @change="swipeChange" @click="swipeClick($event,index,item.id)">
						<view class="item" @tap.stop="goDetail(item.id,item.state)">
							<view :class="item.state == 0 ? 'circle' : 'circle reads'"></view>
							<view class="message">
								<view class="warn">
									<image src="../../static/images/xitongtongzhi.png"></image>
									<view class="warn-title" v-if="item.type == 1">系统通知</view>
									<view class="warn-title" v-if="item.type == 2">订单消息</view>
								</view>
