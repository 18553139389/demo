			...
			<view class="boxs">
				...
				@emptyclick="emptyClick" @topclick="topClick">
					<view class="item" v-for="(v,k) in list" :key="k">
						<view class="item-top">
							<view style="width: 75%;">{{v.certificatename}}</view>
							<view class="item-state" v-if="v.state == 0">待处理</view>
							<view class="item-state" v-if="v.state == 1">待处理</view>
							<view class="item-state" v-if="v.state == 2" style="background: #FF4848;">已失效</view>
						</view>
						<view class="item-middle" v-if="navIndex == 2">预算：￥{{v.money}}</view>
						<view class="item-middle" v-if="navIndex == 1">微信：{{v.weixin}}</view>
						<view class="item-bottom">
							<view>{{v.name}} {{v.phone}}</view>
							<image src="../../static/images/empty.png" v-if="v.state == 2" @tap="goDel(v.bid)"></image>
						</view>
					</view>
				</mescroll-uni>
			</view>
