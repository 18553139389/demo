	...
	<view class="contain">
		<view class="fixed">
			<cu-custom :isBack="true" :Color="Color" :isIcon="true" bgColor="bg-shadeTop text-white">
				...
				<block slot="content">平台订单</block>
			</cu-custom>
		</view>
		<view class="wrapper">
			<view class="search">
				<image src="../../static/images/search.png"></image>
				<input type="text" v-model="val" placeholder="请输入订单号或手机号">
				<view class="look" @tap="goLook">查询</view>
			</view>
		</view>
		<view class="all">
			<view class="historys" v-if="show">
				<view class="history">
					<view>搜索历史：</view>
				</view>
				<ul class="lists">
					<li v-for="(v,k) in lists" :key="k" v-if="k < 6" @tap="goKey(v)">{{v}}</li>
				</ul>
			</view>
			<view class="items">
				...
				@emptyclick="emptyClick" @topclick="topClick">
					<view class="list-item" @tap="goDetail(v.ordernum)" v-for="(v,k) in list" :key="k">
						...
						<view class="list-right">
							...
							<view class="list-state" style="width:120rpx;text-align:center;padding:8rpx 0;border-radius: 4px;background: #EAF5FF;color: #249AFD;" v-if="v.type == 2">办证</view>
							<view class="list-state" style="width:120rpx;text-align:center;padding:8rpx 0;border-radius: 4px;background: #EAF5FF;color: #249AFD;" v-if="v.type == 3">转让</view>
	...
	export default {
		...
		onLoad() {
			if (uni.getStorageSync('history')) {
				this.lists = uni.getStorageSync('history')
			} else {
