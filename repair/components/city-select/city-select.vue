	...
	<view class="city-select">
		<scroll-view :scroll-top="scrollTop" scroll-y="true" class="city-select-main" id="city-select-main" :scroll-into-view="toView">
			...
			<view class="city-serach" v-if="isSearch">
				<image src="../../static/images/search1.png" style="width: 34rpx;height: 34rpx;margin-right: 20rpx;"></image>
				<input @input="keyInput" placeholder="请输入城市名称" class="city-serach-input" />
			</view>
			<!-- 当前定位城市 -->
			<view class="hot-title" v-if="activeCity && !serachCity">当前定位城市</view>
			<view class="hot-city" v-if="activeCity && !serachCity">
				<view class="hot-item location" @click="cityTrigger(activeCity)">
					<image src="../../static/images/current.png" style="width: 42rpx;height: 42rpx;"></image>
					<text>{{ activeCity[formatName] }}</text>
				</view>
			</view>
			<!-- 热门城市 -->
			<view class="hot-title" v-if="hotCity.length > 0 && !serachCity">已开通城市</view>
