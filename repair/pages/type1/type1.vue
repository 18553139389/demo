	...
	<view class="contain">
		...
		<view class="wrapper">
			<view class="item" v-if="hide1">
				<view class="titles">
					<view>服务品类</view>
					<image src="../../static/images/rights.png" @tap="goControl"></image>
				</view>
				<view class="types">
					<view :class="itemIndex5 == k ? 'active types-item' : 'types-item'" v-for="(v,k) in type5" :key="k" :style="{marginRight:(k+1)%4 == 0 ? '0' : '2%'}"
					@tap="changeType5(k,v.id)">{{v.name}}</view>
				</view>
			</view>
			<view class="item" v-if="hide2">
				<view class="titles">
					<view>品类选择</view>
					<image src="../../static/images/rights.png" @tap="goControl1"></image>
				</view>
				<view class="types">
					<view :class="itemIndex6 == k ? 'active types-item' : 'types-item'" v-for="(v,k) in type6" :key="k" :style="{marginRight:(k+1)%4 == 0 ? '0' : '2%'}"
					@tap="changeType6(k,v.id)">{{v.name}}</view>
				</view>
			</view>
			<view class="item" v-if="hide3" @tap="goControl2">
				<view class="titles">
					<view>品牌类型</view>
					<image src="../../static/images/rights.png"></image>
				</view>
				<view class="types" v-if="show3" @tap.stop="">
					<view :class="itemIndex == k ? 'active types-item' : 'types-item'" v-for="(v,k) in type1" :key="k" :style="{marginRight:(k+1)%4 == 0 ? '0' : '2%'}"
					@tap="changeType(k,v.id)">{{v.name}}</view>
				</view>
			</view>
			<view class="item" style="padding-bottom: 0;" v-if="hide4" @tap="goChoice">
				<view class="sku">
					<view class="sku-left">
						<view class="sku-title">型号</view>
						<jPicker ref="picker" class="sku-name" sureColor="#F15A03" position="top" @sure="bookChange" showKey="name" valKey="id"
						:val="bookVal" :options="bookData" />
					</view>
					<image src="../../static/images/rights.png"></image>
				</view>
			</view>
			<view class="item" v-if="hide5">
				<view class="titles" @tap="goControl3">
					<view>内存</view>
					<image src="../../static/images/rights.png"></image>
				</view>
				<view class="types" v-if="show4" @tap.stop="">
			...
			<view class="item" v-if="hide6">
				<view class="titles" @tap="goControl4">
					<view>颜色</view>
					<image src="../../static/images/rights.png"></image>
				</view>
				<view class="types" v-if="show5" @tap.stop="">
					<view :class="itemIndex3 == k ? 'active types-item' : 'types-item'" v-for="(v,k) in type3" :key="k" :style="{marginRight:(k+1)%4 == 0 ? '0' : '2%'}"
					@tap="changeType3(k,v.id,v.price)">{{v.name}}</view>
				</view>
			</view>
			<view class="item" v-if="hide7">
				<view class="titles">
					<view>分类预估价位</view>
					<image src="../../static/images/rights.png"></image>
				</view>
				<view class="choice">
					<radio-group @change="checkboxChange">
						<view class="choice-item" v-for="(v,k) in checkbox" :key="k" v-if="checkbox.length > 0">
							...
							<view class="price">
								<view>￥{{v.price}}</view>
								<radio class='round orange' :class="checkbox[k].state?'checked':''" :checked="checkbox[k].state?true:false"
								:value="v.id"></radio>
							</view>
						</view>
					</radio-group>
				...
				let data = {
					...
					success: function(res) {
						...
						if (res.data.result == 0) {
							...
							if (mark == 1 || mark == 2) {
								self.getSku2(self.bookData[0].id)
							} else if (mark == 3) {
								self.getType2(self.bookData[0].id)
							} else if (mark == 12 || mark == 16) {
								self.getSku3(self.bookData[0].id)
							}
						} else {
							Toast.hideLoading()
							Toast.showToast(res.data.resultNote)
