	...
	<div class="total-wrap" :style="{marginTop: isMainNode?'22rpx':'6rpx'}">
		<div class="node-container">
			...
			<div class="node-container-right" :style="{paddingTop: isMainNode?'0':'8rpx'}">
				...
				<div class="node-desc" :style="{color: isNewest?'red':'#999', marginTop: isMainNode?'10rpx':'0'}">{{acceptStationFixed}}</div>
				<div v-if="nodeData.phone" class="node-phone">{{nodeData.phone}}</div>
				<div class="node-time" :style="{color: isNewest?'red':'#999'}">{{nodeData.AcceptTime}}</div>
			</div>
		</div>
	</div>
</template>
