	...
	<view class="contain">
		...
		<view class="wrapper">
			...
			@emptyclick="emptyClick" @topclick="topClick">
				<ul>
					<li v-for="(v,k) in list" :key="k" @tap="goDetail(v)">
						<view>{{v.adtime}}</view>
						<view class="detail">
							<view class="detail-title" v-if="v.type == 0">系统消息</view>
							<view class="detail-title" v-if="v.type == 1">订单消息</view>
							<view class="detail-news">{{v.title}}</view>
							<view class="look">
								<view class="look-btn">查看详情</view>
	...
	export default {
		...
		methods: {
			...
			goDetail(list) {
				...
				this.$store.commit('changeContent',list.content)
				uni.navigateTo({
					url: `/pages/produce1/produce1?title=${title}`
				})
			}
		}
	}
