	...
	<view class="contain">
		<view class="fixed">
			<cu-custom :isBack="true" :Color="Color" :isIcon="true" bgColor="bg-shadeTop text-white">
				...
				<block slot="content">团队订单</block>
			</cu-custom>
		</view>
		<view class="wrapper">
			<view class="total">累计佣金：￥{{price}}</view>
			<mescroll-uni :down="downOption" @down="downCallback" :up="upOption" @up="upCallback" @init="mescrollInit"
			@emptyclick="emptyClick" @topclick="topClick">
				<ul class="lists">
					<li v-for="(v,k) in list" :key="k">
						<view class="msg">
							<view class="common">用户昵称：{{v.username}}</view>
							<view class="common" v-if="v.type == 1">分销等级：一级</view>
							<view class="common" v-if="v.type == 2">分销等级：二级</view>
							<view class="common">商品名称：{{v.goodsName}}</view>
							<view class="common">订单金额：￥{{v.price}}</view>
							<view class="common">订单编号：{{v.ordernum}}</view>
							<view class="common">下单时间：{{v.adtime}}</view>
						</view>
						<view class="money">预计佣金：￥{{v.commission}}</view>
					</li>
				</ul>
			</mescroll-uni>
		</view>
...
<script>
	...
	export default {
		...
		onLoad(option) {
			this.price = parseFloat(option.price).toFixed(2)
