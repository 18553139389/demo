<template>
	<view class="contain">
		...
		<view class="wrapper">
			<view class="tab">
				<view class="tab-item" v-for="(v,k) in tab" :key="k" :class="itemIndex == k ? 'active' : ''" @tap="changeTab(k)">
					{{v}}
					<span v-if="k == 0">({{num1}})</span>
					<span v-if="k == 1">({{num2}})</span>
				</view>
			</view>
			<mescroll-uni :down="downOption" @down="downCallback" :up="upOption" @up="upCallback" @init="mescrollInit"
			@emptyclick="emptyClick" @topclick="topClick">
				<view class="lists">
					<view class="lis" v-for="(v,k) in list" :key="k">
						<view class="lis-left">
							<img :src="v.userimage.indexOf('https') > -1 ? v.userimage : 'https://www.ytsh.vip/' + v.userimage" alt="">
							<view class="msg">
								<view>{{v.username}}</view>
								<view style="color: #999;font-size: 12px;">注册时间：{{v.adtime}}</view>
							</view>
						</view>
						<view class="lis-right">
							<view>￥{{v.money}}</view>
							<view>{{v.membernum}}个成员</view>
						</view>
					</view>
				</view>
			</mescroll-uni>
