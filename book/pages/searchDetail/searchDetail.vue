	...
	<view class="contain" :class="list.length == 0 ? '' : 'add'">
		...
		<view class="wrapper">
			...
			@emptyclick="emptyClick" @topclick="topClick">
				<ul>
					<li v-for="(v,k) in list" :key="k" @click.stop="goDetail(v.gid)">
						<image :src="v.image" mode="scaleToFill"></image>
						<image v-if="v.stocks == 2" class="no" src="../../static/images/yishouqin.png" mode="scaleToFill"></image>
						<p>{{v.name}}</p>
						<view class="price">
							<view>￥{{v.price}}</view>
							<view class="buy" v-if="v.stocks == 1">购买</view>
							<view class="buy" style="background: #EEEEEE;" v-if="v.stocks == 2">购买</view>
						</view>
					</li>
				</ul>
			</mescroll-uni>
...
<script>
	...
	export default {
		...
		methods: {
			...
			goDetail(id) {
				uni.navigateTo({
					url: '/pages/detail/detail?id=' + id + '&uid=' + uni.getStorageSync('uid')
				})
			}
		}
	}
