		...
		<view class="wrapper">
			...
			@emptyclick="emptyClick" @topclick="topClick">
				<view class="list" v-for="(v,k) in list" :key="k">
					<view class="time">{{v.endtime}}</view>
					<view class="detail">
						<view class="d-title">提现</view>
						<view class="d-money">
							<view class="d-state" v-if="v.state == 0">本次提现（待审核）</view>
							<view class="d-state" v-if="v.state == 1">本次提现（审核通过）</view>
							<view class="d-state" v-if="v.state == 2">本次提现（审核拒绝）</view>
							<view class="d-price">-{{v.money}}</view>
						</view>
						<view class="d-time">
							<view class="d-left">申请提现时间：</view>
							<view class="d-right">{{v.adtime}}</view>
						</view>
						<view class="d-time" v-if="v.state == 2" style="padding-bottom: 24rpx;">
							<view class="d-left">提现拒绝理由：</view>
							<view class="d-right">{{v.reason}}</view>
						</view>
					</view>
				</view>
			</mescroll-uni>
