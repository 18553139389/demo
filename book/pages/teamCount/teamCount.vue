	...
	<view class="contain">
		<view class="fixed">
			<cu-custom :isBack="true" :Color="Color" :isIcon="true" bgColor="bg-shadeTop text-white">
				...
				<block slot="content">业绩统计</block>
			</cu-custom>
		</view>
		<view class="wrapper">
			<view class="count">
				<view>订单总额：￥{{order.ordermoney}}</view>
				<view>订单数量：{{order.ordercount}}</view>
			</view>
				...
				<view class="list">
					<view class="list-item" v-for="(v,k) in list" :key="k">
						<view class="list-top">
							...
							<view class="state" v-if="v.state == 1">待收款</view>
							<view class="state" v-if="v.state == 2">待上架</view>
							<view class="state" v-if="v.state == 3">已委托</view>
							<view class="state" v-if="v.state == 4">已完成</view>
							<view class="state" v-if="v.state == 5">已删除</view>
							<view class="state" v-if="v.state == 6">已申诉</view>
							<view class="state" v-if="v.state == 7">已取消</view>
						</view>
						<view class="middle">
						...
						<view class="common">订单编号：<span style="color: #999;">{{v.ordernum}}</span></view>
						<view class="common">下单时间：<span style="color: #999;">{{v.adtime}}</span></view>
						<view class="common">订单金额：<span style="color: #999;">￥{{v.price}}</span></view>
					</view>
				</view>
			</mescroll-uni>
		</view>
						...
						success: function(res) {
							...
							if (res.data.result == 0) {
								...
								if(res.data.dataobject) {
									self.order = res.data.dataobject
								}
