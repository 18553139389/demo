	...
	<view class="contain">
		...
		<mescroll-uni :down="downOption" @down="downCallback" :up="upOption" @up="upCallback" @init="mescrollInit"
		 @emptyclick="emptyClick" @topclick="topClick">
			<view class="lists">
				<view class="list" v-for="(v,k) in list" :key="k" @tap="goDetail(v.orderId)">
					<view class="item">
						<view>订单编号：{{v.orderId}}</view>
					</view>
					<view class="item">
						<view>{{v.hotelName}}</view>
					</view>
					<view class="item">
						<view>{{v.roomName}}</view>
						<view style="color: #DE2910;">￥{{v.amount}}</view>
					</view>
					<view class="item" style="border-bottom: none;">
						<view>入住时间：{{v.inDate}}</view>
						<view style="color: #DE2910;" v-if="v.state == 2">待付款</view>
						<view style="color: #DE2910;" v-if="v.state == 3">已付款</view>
						<view style="color: #DE2910;" v-if="v.state == 4">已入住</view>
						<view style="color: #DE2910;" v-if="v.state == 5">已取消</view>
						<view style="color: #DE2910;" v-if="v.state == 6">取消中</view>
						<view style="color: #DE2910;" v-if="v.state == 7">已退款</view>
						<view style="color: #DE2910;" v-if="v.state == 8">已离店</view>
						<view style="color: #DE2910;" v-if="v.state == 13">待确认</view>
						<view style="color: #DE2910;" v-if="v.state == 14">已确认</view>
						<view style="color: #DE2910;" v-if="v.state == 20">待入住</view>
						<view style="color: #DE2910;" v-if="v.state == 23">未入住</view>
					</view>
				</view>
			</view>
		</mescroll-uni>
...
<script>
	...
	export default {
		...
		methods: {
			...
			goDetail(id) {
				uni.navigateTo({
					url: '../hotelOrderDetail/hotelOrderDetail?id='+id
				})
			}
		}
	}
