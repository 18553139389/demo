<template>
	<view class="contain">
		...
		<model :show="show" title="退单须知" :content="'距离预约时间'+refundList.hours+'小时以上退单需要扣除'+parseInt(refundList.front)+'%违约金，距离预约时间'+parseInt(refundList.hours)+'小时内需要扣除'+parseInt(refundList.behind)+'%违约金。 订单超时需要扣除'+parseInt(refundList.expire)+'%违约金。'" :align="left"
	...
	export default {
		...
		methods: {
			...
			confirm() {
				...
				let data = {
					url: '/api/gzh/cancelViphallOrder',
					data: datas,
					success: function(res) {
						console.log(res)
						if (res.data.result == 0) {
			...
			goService() {
				// uni.navigateTo({
				// 	url: '../assistant/assistant?list=' + JSON.stringify(this.serviceList)
				// })
				let self = this
				uni.makePhoneCall({
