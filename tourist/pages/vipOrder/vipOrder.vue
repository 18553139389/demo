	...
	<view class="contain">
		<view class="fixed">
			<cu-custom :isBacks="true" :Color="Color" :backColor="backColor" :isIcons="true" bgColor="bg-shadeTop text-white" @Back="Back">
				...
				<block slot="content">贵宾厅订单</block>
			</cu-custom>
		</view>
		<!--筛选条件; 模拟列表的重置和演示空布局-->
		<view class="nav">
				...
				<block v-for="(v,k) in listData" :key="k" v-if="v.state == 1">
					<view class="list" @tap="goDetail(v.orderId)">
						...
						<view class="message" style="padding-top: 20upx;">
							<view class="msgOrder">
								<text style="margin-bottom: 8upx;">订单号：{{v.orderId}}</text>
								<text style="margin-bottom: 8upx;">到厅时间：{{v.reserveDate}}</text>
...
<script>
	...
	export default {
		...
		methods: {
			...
			goDetail(id) {
				uni.navigateTo({
					url: '../vipRoomOrder/vipRoomOrder?id=' + id
				})
			},
