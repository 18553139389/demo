			...
			<view class="head-body">
				...
				<view>
					<view class='pr40' style="display: inline-block;">
						<uni-icon type="arrowup" size='26' color='#333' @click='prevDeta' />
					</view>
					<view style="display: inline-block;">
						<uni-icon type="arrowdown" size='26' color='#333' @click='nextDate' />
					</view>
				</view>
			</view>
	...
	export default {
		...
		methods: {
			...
			ChoiceDate(index, disable) {
				...
				_date = {
					date: `${this.swiperData.month}-${day}`,
					all: `${this.swiperData.year}-${this.swiperData.month}-${day}`
				}
				if (disable != true) {
					// 添加数据
	...
	.content {
		...
		.body {
			...
			.mian-body {
				...
				.mian-bolck {
					width: calc(750upx / 7 - 10upx);
					padding: 8upx 0;
					min-height: calc(750upx / 7 - 10upx);
					box-sizing: border-box;
					font-size: 34upx;
					display: flex;
					flex-flow: row wrap;
