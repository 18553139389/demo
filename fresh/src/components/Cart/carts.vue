...
export default {
  ...
  created() {
    ...
    this.tocar = JSON.parse(localStorage.getItem("tocar"));
  },
  components: {
    Tab,
    Tabs
  ...
  mounted() {
    ...
    mui.back = function() {
      if (!first) {
        self.$router.push("/" + self.tocar);
        localStorage.removeItem("tocar");
