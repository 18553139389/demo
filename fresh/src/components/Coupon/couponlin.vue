...
export default {
  ...
  created() {
    ...
    this.myCoupon(this.$route.params.id);
  },
  computed: {
    jiage() {
      let countlu = 0;
      for (let i = 0; i < this.$route.params.item.length; i++) {
        countlu +=
          Number(this.$route.params.item[i].new) *
          Number(this.$route.params.item[i].number);
      }
      return countlu;
    }
  },
  ...
  methods: {
    ...
    getCoupon(state, id) {
      ...
      you.num = state;
      you.id = id;
      you.sid = this.$router.history.current.params.id;
