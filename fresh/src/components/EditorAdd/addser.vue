		...
		<div class="box">
			<ul>
				...
				<li style="justify-content:center">
					<div class="common" :class="{active:itemIndex == k}" v-for="(v,k) in sex" :key="k" @click="change(k)">{{v}}</div>
				</li>
				<li>
					<span>手机号：</span>
					<input type="text" placeholder="请输入联系人的电话" @blur="blur2" v-model="phone" />
				</li>
				<li @click="showAdd">
					<span>地址栏：</span>
					<input type="text" placeholder="请选择您的地理位置" disabled="disabled" id="cityName" style="background: none;" v-model="addr" />
	...
	export default {
		...
		created() {
			...
			if (JSON.parse(localStorage.getItem("adduser")) !== null) {
				...
				this.addID = JSON.parse(localStorage.getItem("adduser")).addID;
				this.addr = JSON.parse(localStorage.getItem("adduser")).address.address;
				this.lon = JSON.parse(localStorage.getItem("adduser")).address.lng;
		...
		methods: {
				...
				Request.postRequest(add)
					.then(res => {
						...
						if (res.data.result == "0") {
							this.addr = res.data.province + res.data.city;
							this.addID = res.data.id; //地址id
							this.lat = res.data.lat;
							this.lon = res.data.lon;
							this.name = res.data.receiverName; //收货人姓名
							this.phone = res.data.mobile; //手机号
							this.itemIndex = res.data.call; //先生/女士
							this.province = res.data.province; //省
							this.city = res.data.city; //市
							this.town = res.data.town; //区
							this.add = res.data.addr; //详细地址
							if (res.data.default == 0) {
								//是否是默认地址
								this.checked = false;
							} else {
								this.checked = true;
			...
			showAdd() {
				...
				adduser.addID = this.addID;
				adduser.ID = this.$route.query.id;
