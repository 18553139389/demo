...
export default {
  ...
  created() {
    if (JSON.parse(localStorage.getItem("shang-shopID")) !== null) {
      this.id = JSON.parse(localStorage.getItem("shang-shopID")).id;
    }
  ...
  mounted() {
    ...
    mui.back = function() {
      if (!first) {
        self.$router.push("/shop_detailshang");
  ...
  methods: {
    ...
    order() {
      ...
      localStorage.setItem("tocar", JSON.stringify("goods_Detailshang"));
    },
    onAdd(data) {
      if (this.cid == null) {
        this.$router.push("/login");
    ...
    onBuyClicked(data) {
      if (this.cid == null) {
        ...
        if (data.selectedSkuComb.stock_num <= 0) {
          ...
          localStorage.setItem("Torders", JSON.stringify("goods_Detailshang"));
        }
      }
    },
    tiaozhuan(id, name) {
