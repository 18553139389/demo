...
export default {
  ...
  created() {
    if (this.$route.params.id !== undefined) {
      ...
      this.id = JSON.parse(localStorage.getItem("fen-shopID")).id;
    }
  ...
  methods: {
    ...
    shopdian() {
      ...
      let ID = {};
      ID.id = this.dain;
      localStorage.setItem("fen-dianID", JSON.stringify(ID));
    },
    uinit() {
      let pin = {
        cmd: "goodsComment",
    ...
    order() {
      ...
      localStorage.setItem("tocar", JSON.stringify("goods_detailfen"));
    },
    onAdd(data) {
      if (this.cid == null) {
        this.$router.push("/login");
    ...
    onBuyClicked(data) {
      if (this.cid == null) {
        ...
        if (data.selectedSkuComb.stock_num <= 0) {
          ...
          localStorage.setItem("Torders", JSON.stringify("goods_detailfen"));
        }
      }
    },
    tiaozhuan(id, name) {
