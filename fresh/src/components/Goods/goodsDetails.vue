...
export default {
  ...
  created() {
    if (this.$route.params.id !== undefined) {
      ...
      this.id = JSON.parse(localStorage.getItem("cart-shopID")).id;
    }
  ...
  methods: {
    ...
    shopdian() {
      ...
      let IDs = {};
      IDs.id = this.dain;
      localStorage.setItem("dianIDs", JSON.stringify(IDs));
    },
    uinit() {
      let pin = {
        cmd: "goodsComment",
    ...
    order() {
      ...
      localStorage.setItem("tocar", JSON.stringify("goods_details"));
    },
    onAdd(data) {
      if (this.cid == null) {
        this.$router.push("/login");
    ...
    onBuyClicked(data) {
      if (this.cid == null) {
        ...
        if (data.selectedSkuComb.stock_num <= 0) {
          ...
          localStorage.setItem("Torders", JSON.stringify("goods_details"));
        }
      }
    },
    tiaozhuan(id, name) {
