...
export default {
  ...
  created() {
    if (this.$route.params.id !== undefined) {
      ...
      this.id = JSON.parse(localStorage.getItem("xiang-shop1ID")).id;
    }
  ...
  methods: {
    ...
    back() {
      this.$router.push({
        path: "/order_detail",
        query: {
          id: this.$route.params.ding
        }
      });
    },
    order() {
      this.$router.push("/carts");
      localStorage.setItem("tocar", JSON.stringify("goodsxiang"));
    },
    onAdd(data) {
      if (this.cid == null) {
        this.$router.push("/login");
    ...
    onBuyClicked(data) {
      if (this.cid == null) {
        ...
        if (data.selectedSkuComb.stock_num <= 0) {
          ...
          localStorage.setItem("Torders", JSON.stringify("goodsxiang"));
        }
      }
    },
    tiaozhuan(id, name) {
