...
export default {
  ...
  created() {
    if (this.$route.params.id !== undefined) {
      ...
      this.id = JSON.parse(localStorage.getItem("near-shopID")).id;
    }
  ...
  methods: {
    ...
    order() {
      ...
      localStorage.setItem("tocar", JSON.stringify("goods_detailnear"));
    },
    onAdd(data) {
      if (this.cid == null) {
        this.$router.push("/login");
    ...
    onBuyClicked(data) {
      if (this.cid == null) {
        ...
        if (data.selectedSkuComb.stock_num <= 0) {
          ...
          localStorage.setItem("Torders", JSON.stringify("goods_detailnear"));
        }
      }
    },
    tiaozhuan(id, name) {
