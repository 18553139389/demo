...
export default {
  ...
  methods: {
    ...
    shopdian() {
      ...
      let IDs = {};
      IDs.id = this.dain;
      localStorage.setItem("fir-dianID", JSON.stringify(IDs));
    },
    uinit() {
      let pin = {
        cmd: "goodsComment",
    ...
    order() {
      ...
      localStorage.setItem("tocar", JSON.stringify("Goodsfirshop"));
    },
    onAdd(data) {
      if (this.cid == null) {
        this.$router.push("/login");
    ...
    onBuyClicked(data) {
      if (this.cid == null) {
        ...
        if (data.selectedSkuComb.stock_num <= 0) {
          ...
          localStorage.setItem("Torders", JSON.stringify("Goodsfirshop"));
        }
      }
    },
    tiaozhuan(id, name) {
