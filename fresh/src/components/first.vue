	...
	<div class="contain" v-cloak>
		...
		<div ref="all" class="wrapper">
			<van-tabs :active="active" color="rgb(114,209,65)" title-active-color="rgb(114,209,65)" sticky :offset-top="60"
			 @click="changeTab" swipeable animated :duration="0.5" @change="changeIng">
				<van-tab :title="v.name" v-for="(v,k) in cate1" :key="k">
					<van-list v-model="loading" :finished="finished" finished-text="没有更多了" @load="onLoad" :offset="10">
						<van-pull-refresh v-model="isLoading" @refresh="onRefresh">
							...
							<div class="slide-box">
								<discount :recommendList="list" :title="title" v-if="cate1[active].name=='推荐'" @shopcart="shopcart"></discount>
								<discount :recommendList="dataList" :title="title1" v-if="cate1[active].name!=='推荐'" @shopcart="shopcart"></discount>
	...
	export default {
		...
		created() {
			...
			if (JSON.parse(localStorage.getItem("first")) !== null) {
				// this.active = JSON.parse(localStorage.getItem("first")).num;
				this.id = "";
				this.active = 0;
				let obj = {};
				obj.num = this.active;
		...
		methods: {
			...
			diao() {
				...
				self.donghua = true;
				//调用一级分类导航
				if (JSON.parse(localStorage.getItem("cateTAB"))) {
					self.cate1 = JSON.parse(localStorage.getItem("cateTAB"));
				}
				let datas = {
				...
				Request.postRequest(datas)
					.then(res => {
						...
						if (res.data.result == 0) {
							...
							localStorage.setItem("cateTAB", JSON.stringify(self.cate1));
							if (JSON.parse(localStorage.getItem("first"))) {
								self.id = JSON.parse(localStorage.getItem("first")).id;
								self.active = JSON.parse(localStorage.getItem("first")).num;
							} else {
			...
			initLoad(id) {
				//我是来调取首页内容的
				if (id == "") {
					if (JSON.parse(localStorage.getItem("rotationCharts"))) {
						this.images = JSON.parse(localStorage.getItem("rotationCharts"));
					}
					if (JSON.parse(localStorage.getItem("childs"))) {
						this.datas = JSON.parse(localStorage.getItem("childs"));
					}
					if (JSON.parse(localStorage.getItem("discount"))) {
						this.list = JSON.parse(localStorage.getItem("discount"));
					}
					if (JSON.parse(localStorage.getItem("orders"))) {
						this.recommendList = JSON.parse(localStorage.getItem("orders"));
					}
				} else {
					if (JSON.parse(localStorage.getItem("rotationChart"))) {
						this.images = JSON.parse(localStorage.getItem("rotationChart"));
					}
					if (JSON.parse(localStorage.getItem("child"))) {
						this.datas = JSON.parse(localStorage.getItem("child"));
					}
					if (JSON.parse(localStorage.getItem("discount"))) {
						this.dataList = JSON.parse(localStorage.getItem("discount"));
					}
					if (JSON.parse(localStorage.getItem("order"))) {
						this.recommendList = JSON.parse(localStorage.getItem("order"));
					}
				}
					...
					.then(res => {
						...
						if (res.data.result == 0) {
							if (id == "") {
								...
								localStorage.setItem("childs", JSON.stringify(self.datas));
								...
								localStorage.setItem(
									"orders",
									JSON.stringify(self.recommendList)
								);
							} else {
								self.datas = res.data.child;
								localStorage.setItem("child", JSON.stringify(self.datas));
								self.images = res.data.rotationChart;
								localStorage.setItem(
									"rotationChart",
									JSON.stringify(self.images)
								...
								if (self.dataList !== undefined) {
									localStorage.setItem("discount", JSON.stringify(self.dataList));
								}
								self.totalPage = res.data.totalPage;
								self.recommendList = res.data.order;
								localStorage.setItem("order", JSON.stringify(self.recommendList));
							}
							self.donghua = false;
						}
					})
