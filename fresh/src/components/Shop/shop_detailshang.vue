...
export default {
  ...
  created() {
    ...
    if (JSON.parse(localStorage.getItem("shangID")) !== null) {
      this.id = JSON.parse(localStorage.getItem("shangID")).id;
    }
  },
  mounted() {
    this.init();
  ...
  methods: {
    ...
    goGoods(id) {
      console.log(id);
      this.$router.push("/goods_Detailshang");
      let ID = {};
      ID.id = id;
      localStorage.setItem("shang-shopID", JSON.stringify(ID));
    },
    onLoad: function() {
      let datas = {
        cmd: "selectShopGoods",
