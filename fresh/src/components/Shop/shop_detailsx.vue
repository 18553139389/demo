...
export default {
  ...
  created() {
    ...
    if (this.$route.params.id !== undefined) {
      ...
      this.id = JSON.parse(localStorage.getItem("first-dianID")).id;
    }
  },
  mounted() {
    this.init();
    ...
    mui.back = function() {
      if (!first) {
        self.$router.push("/goods_detailsx");
  ...
  methods: {
    ...
    goGoods(id) {
      ...
      let ID = {};
      ID.id = id;
      localStorage.setItem("first-shop1ID", JSON.stringify(ID));
    },
    onLoad: function() {
      let datas = {
        cmd: "selectShopGoods",
    ...
    back() {
      this.$router.push("/goods_detailsx");
    },
