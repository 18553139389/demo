...
export default {
  ...
  created() {
    ...
    if (this.$route.params.id !== undefined) {
      ...
      this.id = JSON.parse(localStorage.getItem("s-dianID")).id;
    }
  },
  mounted() {
    this.init();
  ...
  methods: {
    ...
    goGoods(id) {
      ...
      let ID = {};
      ID.id = id;
      localStorage.setItem("s-shopID", JSON.stringify(ID));
    },
    onLoad: function() {
      let datas = {
        cmd: "selectShopGoods",
