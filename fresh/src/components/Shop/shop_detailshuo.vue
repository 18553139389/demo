...
export default {
  ...
  created() {
    ...
    if (this.$route.params.id !== undefined) {
      ...
      this.id = JSON.parse(localStorage.getItem("shuo-dianID")).id;
    }
  },
  mounted() {
    this.init();
  ...
  methods: {
    ...
    goGoods(id) {
      ...
      let ID = {};
      ID.id = id;
      localStorage.setItem("shou-shop1ID", JSON.stringify(ID));
    },
    onLoad: function() {
      let datas = {
        cmd: "selectShopGoods",
