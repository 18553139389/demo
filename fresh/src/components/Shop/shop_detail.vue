...
export default {
  ...
  created() {
    ...
    } else if (JSON.parse(localStorage.getItem("zhe-dianID")) !== null) {
      this.id = JSON.parse(localStorage.getItem("zhe-dianID")).id;
    } else {
      this.id = JSON.parse(localStorage.getItem("dianID")).id;
    }
  },
  mounted() {
    this.init();
  ...
  methods: {
    ...
    goGoods(id) {
      ...
      let ID = {};
      ID.id = id;
      localStorage.setItem("near-shopID", JSON.stringify(ID));
    },
    onLoad: function() {
      let datas = {
        cmd: "selectShopGoods",
