	...
	<view class="contain">
		...
		<view class="wrapper">
			...
			<picker @change="bindPickerChange2" :value="index2" :range="array2" range-key="name">
				<view class="question1" style="margin-top: 30rpx;">
					<input type="text" v-model="val3" placeholder="请选择" disabled>
					<image src="../../static/images/san.png"></image>
				</view>
			</picker>
			<view class="section">
				<!-- <input type="text" v-model="tit" placeholder="咨询标题"> -->
				<textarea v-model="val" placeholder="请输入反映内容(不超过500字)" />
				<view class="upload">
...
<script>
	...
	export default {
		...
		methods: {
			init() {
				...
				let data = {
					...
					success: function(res) {
						...
						if (res.data.success) {
							Toast.hideLoading()
							self.array2 = res.data.body.maps
						} else {
							Toast.hideLoading()
			...
			bindPickerChange2(e) {
				this.val3 = this.array2[e.detail.value].name
				this.ids = this.array2[e.detail.value].id
			},
			showPopup() {
