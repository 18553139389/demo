	...
	<view class="contain">
		<view class="fixed">
			<cu-custom :isBack="true" :Color="Color" :isIcon="true" bgColor="bg-shadeTop text-white">
				...
				<block slot="content">我的问题反映</block>
			</cu-custom>
		</view>
		<view class="wrapper">
			...
			@emptyclick="emptyClick" @topclick="topClick">
				<view class="item" v-for="(v,k) in list" :key="k">
					...
					<view class="address">
						<view class="nick">焦作市：</view>
						<view class="detail">{{v.address}}</view>
					</view>
					<view style="margin-top: 24rpx;">类型项： {{v.proName}}</view>
					<view class="address">
						<view class="nick" style="width: 23%;">当前状态：</view>
			...
			init(mescroll) {
				...
				let data = {
					...
					success: function(res) {
						...
						if (res.data.success) {
							if (res.data.body.maps) {
								for(let i=0;i<res.data.body.maps.length;i++) {
									self.list.push(res.data.body.maps[i])
								}
							}
						} else {
						...
						let curPageData = []
						if (res.data.body.maps) {
							curPageData = res.data.body.maps
						}
						// 接口返回的总页数 (比如列表有26个数据,每页10条,共3页; 则totalPage值为3)
						let totalPage = 1
						if (res.data.body.totalPage) {
