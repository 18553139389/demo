<template>
	<view class="contain">
		<view class="wrapper">
			<view class="tab">
				<view class="tab-item" v-for="(v,k) in tabs" :key="k" :class="itemIndex == k ? 'active' : ''" @tap="change(k)">{{v}}</view>
			</view>
			<mescroll-uni :down="downOption" @down="downCallback" :up="upOption" @up="upCallback" @init="mescrollInit"
			@emptyclick="emptyClick" @topclick="topClick">
				<view class="lists">
					<view class="item" v-for="(v,k) in list" :key="k" @tap="goDetail(v.orderId)">
						<view class="list-top">
							<view>{{v.orderId}}</view>
							<view v-if="v.state == 0">待完成</view>
							<view v-if="v.state == 1">已完成</view>
							<view v-if="v.state == 2">订单无效</view>
						</view>
						<view class="list-middle">
							<img :src="v.productImage" alt="">
							<view class="product">
								<view>{{v.productTitle}}</view>
								<view class="price">
									<view style="color: red;">￥{{v.price}}</view>
									<view style="margin-left: 40upx;">x{{v.qty}}</view>
								</view>
							</view>
						</view>
						<view class="list-top">
							<view>{{v.createDate}}</view>
							<view>合计：<span style="color: red;">￥{{v.amount}}</span></view>
						</view>
					</view>
				</view>
			</mescroll-uni>
