	...
	<view class="contain">
		...
		<view class="wrapper">
			<view class="top">
				<view class="top-left">
					<view>总余额</view>
					<view style="font-size: 60upx;margin-top: 20upx;">{{lists.balance}}</view>
				</view>
				<view class="top-left">
					<view>提现中：{{lists.cash}}</view>
					<view style="margin-top: 16upx;">待返款：{{lists.refund}}</view>
				</view>
			</view>
			<view class="lists">
				<view class="item" style="height: 90upx;">
					<view style="font-size: 30upx;">余额明细</view>
				</view>
				<mescroll-uni :down="downOption" @down="downCallback" :up="upOption" @up="upCallback" @init="mescrollInit"
				@emptyclick="emptyClick" @topclick="topClick" style="margin-bottom: 120upx;">
					<view class="item" v-for="(v,k) in list" :key="k">
						<view class="item-left">
							<view style="font-size: 30upx;margin-bottom: 16upx;">{{v.title}}</view>
							<view style="color: #666;">{{v.createDate}}</view>
						</view>
						<view class="item-right" v-if="v.type == 0">-￥{{v.amount}}</view>
						<view class="item-right" v-if="v.type == 1">+￥{{v.amount}}</view>
					</view>
				</mescroll-uni>
				<view class="submit" @tap="goUse">提现</view>
			</view>
		</view>
	</view>
</template>
...
<script>
	...
	export default {
		...
		methods: {
			...
			goUse() {
				uni.navigateTo({
				    url: '/pages/canUse/canUse?money=' + this.lists.balance
				})
			}
		}
	}
