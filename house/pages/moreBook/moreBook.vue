	...
	<view class="contain">
		<view class="fixed">
			<cu-custom :isBacks="true" bgColor="bg-shadeTop" @goBack="goBack">
				<block slot="content">更多小说</block>
			</cu-custom>
		</view>
		<view class="wrapper">
			<view class="search-box" @tap="goSearch">
				<image src="../../static/images/search.png" mode="widthFix"></image>
				<input type="text" v-model="value" placeholder="请输入关键字进行搜索" disabled>
			</view>
...
<script>
	...
	export default {
		...
		onLoad() {
			// 小说分类
			this.$u.post('getnovelsclassi').then(res => {
				console.log(res)
				this.list = res.dataList
				this.ids = res.dataList[0].cid
