	...
	<view class="contain">
		...
		<mescroll-uni :down="downOption" @down="downCallback" :up="upOption" @up="upCallback" @init="mescrollInit"
		 @emptyclick="emptyClick" @topclick="topClick">
			<view class="list">
				<view class="module1" v-for="(v,k) in list" :key="k">
					<view class="column">
						<view class="item" @tap="goDetail(v.ordernum)">
							<image :src="v.goodsimage"></image>
							<view class="message">
								<view class="msg-title">{{v.goodsname}}</view>
								<view class="price">
									<view>{{v.goodsprice}}元</view>
									<view class="btn" v-if="v.state == 1">待发货</view>
									<view class="btn" v-if="v.state == 2">已发货</view>
								</view>
							</view>
						</view>
					</view>
	...
	export default {
		...
		methods: {
			...
			init(mescroll) {
				...
				this.$u.post('getmygoodsorderlist', data1).then(res => {
					console.log(res)
					if (res.result == 0) {
						if (res.dataList) {
