		 ...
		 @emptyclick="emptyClick" @topclick="topClick">
			<view class="list">
				<view class="item" v-for="(v,k) in list" :key="k">
					<image :src="v.limage"></image>
					<view class="name">{{v.lname}}</view>
					<view class="info">
						<view class="left">
							<view>{{v.adtime}}</view>
							<view style="margin-left: 30rpx;">{{v.lgrades}}</view>
						</view>
						<view style="color: #fe5f23;" v-if="v.state == 0">待开奖</view>
						<view style="color: #fe5f23;" v-if="v.state == 1">未中奖</view>
						<view style="color: #fe5f23;" v-if="v.state == 2">已中奖</view>
						<view style="color: #fe5f23;" v-if="v.state == 3">已中奖</view>
						<view style="color: #fe5f23;" v-if="v.state == 4">已中奖</view>
					</view>
				</view>
			</view>
		</mescroll-uni>
	...
	export default {
		data() {
			return {
				...
				upOption: {
					...
					empty: {
						use: true, // 是否显示空布局
						icon: "../../static/images/none.png", // 图标路径
						tip: '暂时还没有抽奖记录哦~', // 提示
						// btnText: '去逛逛 >' // 按钮
					}
				}
		...
		methods: {
			...
			init(mescroll) {
				...
				this.$u.post('getmyluckdrawrecord', data1).then(res => {
					console.log(res)
					if (res.result == 0) {
						if (res.dataList) {
