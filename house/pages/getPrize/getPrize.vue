		 ...
		 @emptyclick="emptyClick" @topclick="topClick">
			<view class="list">
				<view class="item" v-for="(v,k) in list" :key="k">
					...
					<view class="address" v-if="v.state == 2" @tap="goAdress(v.irid)">待填写地址</view>
					<view class="address" style="background: #ccc;" v-if="v.state == 3">已填写</view>
					<view class="address" style="background: #ccc;" v-if="v.state == 4">已填写</view>
				</view>
			</view>
		</mescroll-uni>
	</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			goAdress(id) {
				uni.navigateTo({
				    url: '/pages/personAddress/personAddress?id=' + id
				})
			}
		}
	}
