		 ...
		 @emptyclick="emptyClick" @topclick="topClick">
			<view class="list">
				<view class="item" v-for="(v,k) in list" :key="k">
					<view class="state">
						<view v-if="v.state == 0" style="color: red;">待审核</view>
						<view v-if="v.state == 1">已提现</view>
						<view v-if="v.state == 2">待审拒绝</view>
	...
	export default {
		...
		methods: {
			...
			init(mescroll) {
				...
				this.$u.post('getmemberwithdrawallist', data1).then(res => {
					console.log(res)
					if (res.result == 0) {
						if (res.dataList) {
