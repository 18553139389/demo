	...
	export default {
		...
		mounted() {
			if (localStorage.getItem('sectionList')) {
				this.choiceList = JSON.parse(localStorage.getItem('sectionList'))
			}
			this.init()
		},
		computed: {
		...
		methods: {
			init() {
				...
				let self = this
				Request.postRequest('api/licence/region/list').then(res => {
			...
			submit() {
				...
				localStorage.setItem('mapSection', str)
				localStorage.setItem('sectionList', JSON.stringify(this.choiceList))
				let self = this
				this.$router.push({
					name: 'changeRoute',
					params: {
			...
			goMap() {
				...
				for (let i = 0; i < this.choiceList.length; i++) {
					str += this.choiceList[i].name + ','
					changeIds += this.choiceList[i].id + ','
				}
				str = str.substring(0, str.length - 1)
				changeIds = changeIds.substring(0, changeIds.length - 1)
				localStorage.setItem('mapSection', str)
				localStorage.setItem('changeIds', changeIds)
				localStorage.setItem('sectionList', JSON.stringify(this.choiceList))
				let self = this
				this.$router.push({
					name: 'change_map',
