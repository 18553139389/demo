  ...
  <div class="box">
    ...
    <van-tabs v-model="active" title-active-color="#2076FF" color="#2076FF" :border="false">
      <van-tab title="详情">
        ...
        <div class="con">
          <p class="p1">
            <span class="corg">市场渠道</span> 
            <span>无桩共享电动车招商</span>             
          </p>
          <ul class="ul1">
            <li class="first">任务周期：{{dataobject.days}}天</li><li>{{dataobject.conditions==0?'男':dataobject.conditions==1?'女':'不限'}}</li><li>成交90单</li><li>剩余：100单</li>
          </ul>
          <p class="p2">
            <span><font class="corg">{{dataobject.budgets}}</font>元/单</span> 
            <span class="c9">{{dataobject.looknum}}人浏览</span>           
          </p>
          <p class="p1">任务详情</p>
          <p class="p3">{{dataobject.content1}}</p>
          <div class="detail">
            <div v-for="i in dataobject.images1" :key="i">
              <img :src="i" alt="" class="dimg">
            </div>
            <p>
              <img src="@/assets/images/di.png" alt="" style="width:9px;height:11px;">
              <span style="color:#A5A5A5;">{{dataobject.address}}</span>
            </p>
          </div>
          <p class="p1">任务要求</p>
          <p class="p3">{{dataobject.content2}}</p>
          <div class="detail">
            <div v-for="i in dataobject.images2" :key="i">
              <img :src="i" alt="" class="dimg">
            </div>
...
export default {
  ...
  created() {
    ...
    this.tasksid =this.$route.query.tasksid
    var params ={ cmd: "tasksdetail", uid:this.uid,tasksid:this.tasksid}
    this.postRequest(params).then(res => {       
          // console.log(res)
          this.dataobject = res.data.dataobject
    });
    var params2={ cmd: "tasksfirstmessagelist",uid:this.uid,tasksid:this.tasksid,nowPage:'1',pageCount:'10'}
...
.dimg{width:100px;height: 100px; margin: 8px 0;}
...
.option span{vertical-align:middle;margin-left: 3px;}
</style>
