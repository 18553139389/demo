<template>
	<div class="wrapper">
		...
		<div ref="inputBlock" @click="open()" v-if="defaultType" class="data-show">
			<div :class="['data-show-block', { active: showLength === n }]" v-for="n in 7" :key="n">
			...
			<div v-if="visible" class="keybord-wrap">
				...
				<div class="keybord-keys">
					<ul class="keybord-keys-word-wrap" v-if="keybordType === '字'">
						<li @click.stop="inputWord(item)" class="button" v-for="item in lineOneToThree" :key="item">
							{{ item }}
						</li>
					</ul>
					<div v-else>
						<ul class="keybord-keys-abc-wrap">
							<li @click.stop="inputWord(item)" class="button" v-for="item in abcLineOne" :key="item">
								{{ item }}
							</li>
						</ul>
						<ul class="keybord-keys-abc-wrap">
							<li @click.stop="inputWord(item)" class="button" v-for="item in abcLineTwo" :key="item">
								{{ item }}
							</li>
						</ul>
						<ul class="keybord-keys-abc-wrap-short">
							<li @click.stop="inputWord(item)" class="button" v-for="item in abcLineThree" :key="item">
								{{ item }}
							</li>
						</ul>
					</div>
