		...
		<view class="wrapper">
			<view class="list" style="padding-top: 0;" v-for="(v,k) in list" :key="k">
				<view class="top" :style="{top:top + 'px'}">
					...
					<view class="name">{{v.dw}}({{v.goodsname}})</view>
					<view class="unit">{{v.goodsweight}}{{v.unit}}</view>
				</view>
				<view class="list-item">
					<view class="item" v-for="(ite,ind) in v.items" :key="ind" @tap="goDetail(ite.carnum, k)">
						<view class="item-left">{{ite.carnum}}</view>
...
<script>
	...
	export default {
		...
		onLoad(option) {
			...
			this.dw = option.dw
			this.init(option.date, option.type, option.name,option.dw)
		},
		methods: {
			init(date, type, name, dw) {
				let self = this
				let datas1 = {}
				let url = ''
				if (this.tab == 0) {
					url = '/api/AppDataReport/TypeMonthProductAndProj2TruckList'
					datas1 = {
					...
					url = '/api/AppDataReport/TypeYearProductAndProj2TruckList'
					datas1 = {
						token: uni.getStorageSync('token'),
						addyear: date,
						inorout: type,
			...
			goDetail(car,k) {
				...
				uni.navigateTo({
					url: '/pages/everyList/everyList?name=' + this.name + '&date=' + this.date + '&type=' + this.type + '&tab=' + this.tab + '&car=' + car + '&dw=' + this.dw
				})
			}
		}
	}
