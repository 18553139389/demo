...
<script>
	...
	export default {
		...
		onLoad(option) {
			...
			this.init(option.date, option.name, option.dw)
		},
		methods: {
			init(date, name, dw) {
				let self = this
				...
				let url = ''
				if (this.tab == 0) {
					url = '/api/AppDataReport/TypeMonthDwGoods2TruckList2'
					datas1 = {
					...
					url = '/api/AppDataReport/TypeYearDwGoods2TruckList2'
					datas1 = {
						token: uni.getStorageSync('token'),
						addyear: date,
						inorout: 2,
			...
			goDetail(car,k) {
				...
				uni.navigateTo({
					url: '/pages/everyCar/everyCar?name=' + this.name + '&date=' + this.date + '&tab=' + this.tab + '&car=' + car + '&dw=' + this.dw
				})
			}
		}
	}
