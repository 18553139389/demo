...
! function(t, i) {
	...
	function z() {}
	var W = e ? window.requestAnimationFrame || window.webkitRequestAnimationFrame || window.mozRequestAnimationFrame || window.oRequestAnimationFrame || function(t) {
			return window.setTimeout(t, (t.interval || 100 / 60) / 2)
		} : z,
		A = e ? window.cancelAnimationFrame || window.webkitCancelAnimationFrame || window.mozCancelAnimationFrame || window.oCancelAnimationFrame || function(t) {
	...
	}, V.prototype.transitionTimingFunction = function(t) {
	...
	function nt(t, i) {
		...
		this.options = i, B("function" == typeof this.options.createTombstone, "Infinite scroll need createTombstone Function to create tombstone"), B("function" == typeof this.options.fetch, "Infinite scroll need fetch Function to fetch new data."), B("function" == typeof this.options.render, "Infinite scroll need render Function to render each item."), this.firstAttachedItem = 0, this.lastAttachedItem = 0, this.anchorScrollTop = 0, this.anchorItem = {
			index: 0,
			offset: 0
		}, this.tombstoneHeight = 0, this.tombstoneWidth = 0, this.tombstones = [], this.tombstonesAnimationHandlers = [], this.items = [], this.loadedItems = 0, this.requestInProgress = !1, this.hasMore = !0, this.scroller = t, this.wrapperEl = this.scroller.wrapper, this.scrollerEl = this.scroller.scroller, this.scroller.on("scroll", function() {
			e.onScroll()
	...
	function rt(t, i) {
		this.wrapper = "string" == typeof t ? document.querySelector(t) : t, this.wrapper || N("Can not resolve the wrapper DOM."), this.scroller = this.wrapper.children[0], this.scroller || N("The wrapper need at least one child element to be scroller."), this.scrollerStyle = this.scroller.style, this._init(t, i)
	}
	return nt.prototype.destroy = function() {
		...
		this.onScroll()
	}, nt.prototype.fill = function(t, i) {
	...
	}, nt.prototype.maybeRequestContent = function() {
		var s = this;
	...
	}, nt.prototype.resetMore = function() {
		this.hasMore = !0
	}, nt.prototype._removeTombstones = function() {
		...
		return i
	}, nt.prototype._cleanupUnusedNodes = function(t) {
		for(; t.length;) this.scrollerEl.removeChild(t.pop())
	}, nt.prototype._cacheNodeSize = function() {
	...
	}, nt.prototype._getTombStone = function() {
	...
	}, nt.prototype._addItem = function() {
		this.items.push({
			data: null,
			node: null,
			height: 0,
	...
	}, nt.prototype._calculateAnchoredItem = function(t, i) {
		if(0 === i) return t;
		var e = t.index,
			s = 0;
		if((i += t.offset) < 0) {
	...
	}, (Z = rt).prototype._init = function(t, i) {
	...
	}, Z.prototype._initExtFeatures = function() {
	...
	}, Z.prototype._watchTransition = function() {
	...
	}, Z.prototype._handleAutoBlur = function() {
		this.on("scrollStart", function() {
			var t = document.activeElement;
			!t || "INPUT" !== t.tagName && "TEXTAREA" !== t.tagName || t.blur()
		})
	}, Z.prototype._initDOMObserver = function() {
		var n = this;
	...
	}, Z.prototype._shouldNotRefresh = function() {
	...
	}, Z.prototype._checkDOMUpdate = function() {
		...
		this.enabled = !1
	}, (j = rt).prototype._start = function(t) {
		var i = X[t.type];
		...
		if(this.enabled && !this.destroyed && X[t.type] === this.initiated) {
			this.initiated = !1, this.options.preventDefault && !P(t.target, this.options.preventDefaultException) && t.preventDefault(), this.options.stopPropagation && t.stopPropagation(), this.trigger("touchEnd", {
				x: this.x,
				y: this.y
				...
				if(this._events.flick && n < this.options.flickLimitTime && r < this.options.flickLimitDistance && h < this.options.flickLimitDistance) this.trigger("flick");
					...
					if(this.options.momentum && n < this.options.momentumLimitTime && (h > this.options.momentumLimitDistance || r > this.options.momentumLimitDistance)) {
						...
						var m = this.directionX === R && p || this.directionX === O && d ? this.wrapperWidth : 0,
							...
							g = this.hasHorizontalScroll ? k(this.x, this.startX, n, this.maxScrollX, this.minScrollX, m, this.options) : {
								destination: i,
								duration: 0
							},
							v = this.hasVerticalScroll ? k(this.y, this.startY, n, this.maxScrollY, this.minScrollY, f, this.options) : {
								destination: e,
								duration: 0
							};
	...
	}, j.prototype._transitionEnd = function(t) {
		t.target === this.scroller && this.isInTransition && (this._transitionTime(), (!this.pulling || this.movingDirectionY === C) && !this.resetPosition(this.options.bounceTime, H.bounce) && (this.isInTransition = !1, this.options.probeType !== U && this.trigger("scrollEnd", {
			x: this.x,
			y: this.y
		})))
	}, j.prototype._translate = function(t, i, e) {
		...
		this.isAnimating = !0, A(this.animateTimer),
			function t() {
				...
				var e = l(i = (i - f) / a),
					...
					o = (h - d) * e + d,
					n = (m - u) * e + u;
				c._translate(s, o, n), c.isAnimating && (c.animateTimer = W(t)), c.options.probeType === U && c.trigger("scroll", {
					x: c.x,
					y: c.y
				})
			}()
		...
		var e = 2 < arguments.length && void 0 !== arguments[2] ? arguments[2] : 0,
			s = 3 < arguments.length && void 0 !== arguments[3] ? arguments[3] : H.bounce;
		this.isInTransition = this.options.useTransition && 0 < e && (t !== this.x || i !== this.y), !e || this.options.useTransition ? (this._transitionTimingFunction(s.style), this._transitionTime(e), this._translate(t, i), e && this.options.probeType === U && this._startProbe(), e || t === this.x && i === this.y || (this.trigger("scroll", {
			x: t,
			y: i
		}), this._reflow = document.body.offsetHeight, this.resetPosition(this.options.bounceTime, H.bounce) || this.trigger("scrollEnd", {
			x: t,
			y: i
		...
		var t = window.getComputedStyle(this.scroller, null),
			i = void 0,
			e = void 0;
		return this.options.useTransform ? (i = +((t = t[_.transform].split(")")[0].split(", "))[12] || t[4]), e = +(t[13] || t[5])) : (i = +t.left.replace(/[^-\d.]/g, ""), e = +t.top.replace(/[^-\d.]/g, "")), {
			x: i,
			y: e
		}
		...
		} else !this.options.useTransition && this.isAnimating && (this.isAnimating = !1, A(this.animateTimer), this.trigger("scrollEnd", {
			x: this.x,
			y: this.y
		}), this.stopFromTransition = !0)
	}, j.prototype.destroy = function() {
		this.destroyed = !0, this.trigger("destroy"), this.options.useTransition ? A(this.probeTimer) : A(this.animateTimer), this._removeDOMEvents(), this._events = {}
	}, (G = rt).prototype.on = function(t, i) {
		...
		"string" == typeof g && (g = this.scroller.querySelectorAll(g)), this.on("refresh", function() {
			if(m.pages = [], m.wrapperWidth && m.wrapperHeight && m.scrollerWidth && m.scrollerHeight) {
				var t = f.stepX || m.wrapperWidth,
					...
					c = void 0;
				if(g)
					for(h = g.length, l = -1; r < h; r++) c = M(g[r]), (0 === r || c.left <= M(g[r - 1]).left) && (a = 0, l++), m.pages[a] || (m.pages[a] = []), e = Math.max(-c.left, m.maxScrollX), s = Math.max(-c.top, m.maxScrollY), o = e - Math.round(c.width / 2), n = s - Math.round(c.height / 2), m.pages[a][l] = {
						x: e,
						y: s,
						width: c.width,
						height: c.height,
	...
	}, (J = rt).prototype.wheelTo = function() {
		var t = 0 < arguments.length && void 0 !== arguments[0] ? arguments[0] : 0;
		...
		this.options.scrollX && (r = {
			...
			fade: s,
			interactive: n
		}, this._insertScrollBar(r.el), this.indicators.push(new V(this, r))), this.options.scrollY && (r = {
			el: q("vertical"),
			direction: "vertical",
			fade: s,
			interactive: n
		}, this._insertScrollBar(r.el), this.indicators.push(new V(this, r))), this.on("refresh", function() {
	...
	}, K.prototype._insertScrollBar = function(t) {
		this.wrapper.appendChild(t)
	}, K.prototype._removeScrollBars = function() {
		for(var t = 0; t < this.indicators.length; t++) this.indicators[t].destroy()
	}, (Q = rt).prototype._initPullDown = function() {
		...
		this.pulling = !1, this.resetPosition(this.options.bounceTime, H.bounce)
	...
	}, (tt = rt).prototype._initPullUp = function() {
		this.options.probeType = U, this.pullupWatching = !1, this._watchPullUp()
	}, tt.prototype._watchPullUp = function() {
		this.pullupWatching || (this.pullupWatching = !0, this.on("scroll", this._checkToEnd))
	}, tt.prototype._checkToEnd = function(t) {
		var i = this,
			e = this.options.pullUpLoad.threshold,
			s = void 0 === e ? 0 : e;
		this.movingDirectionY === C && t.y <= this.maxScrollY + s && (this.once("scrollEnd", function() {
			i.pullupWatching = !1
		...
		if(this.enabled) {
			...
			var m = this.y === u;
			this.scrollTo(d, u, a, H.swipe), this.trigger("scroll", {
				x: this.x,
				y: this.y
	...
	}, rt.prototype._initInfinite = function() {
		this.options.probeType = 3, this.maxScrollY = -st, this.infiniteScroller = new nt(this, this.options.infinity)
	}, rt.Version = "1.13.2", rt
});