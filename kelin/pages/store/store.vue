		...
		<view class="wrapper">
			<mescroll-uni :down="downOption" @down="downCallback" :up="upOption" @up="upCallback" @init="mescrollInit"
			 @emptyclick="emptyClick" @topclick="topClick">
				<uni-swipe-action v-for="(v,k) in list" :key="k" @tap="goDetail(v, v.deleted)">
					<uni-swipe-action-item :options="options1" @click.stop="bindClick(v.collectedId, v.infoType, k)">
						<view class="list">
							<img :src="v.coverImage" alt="">
							<view class="right">
								<view class="title">{{v.name}}</view>
								<view class="bottom">
									<view class="mark">{{v.label}}</view>
									<view class="mark" v-if="v.deleted">已删除</view>
								</view>
							</view>
						</view>
		...
		methods: {
			...
			goDetail(item, status) {
				if(!status) {
					if (item.infoType == 1) {
						uni.navigateTo({
							url: `/pages/detail/detail?id=${item.collectedId}`
						})
							...
							let data = {
								...
								success: (res) => {
									if (res.data.success == true) {
										...
										setTimeout(() => {
											uni.navigateTo({
												url: `/pages/album/album?id=${item.collectedId}&list=${JSON.stringify(item.product)}&lists=${JSON.stringify(lists)}`
											})
										}, 300)
									} else {
										Toast.hideLoading()
							...
							let data = {
								...
								success: (res) => {
									if (res.data.success == true) {
										...
										setTimeout(() => {
											uni.navigateTo({
												url: `/pages/train/train?id=${item.collectedId}&list=${JSON.stringify(item.product)}&lists=${JSON.stringify(lists)}`
											})
										}, 300)
									} else {
										Toast.hideLoading()
					...
					} else if (item.infoType == 10) {
						uni.navigateTo({
							url: `/pages/exampleDetail/exampleDetail?id=${item.collectedId}`
						})
					} else if (item.infoType == 11) {
						uni.navigateTo({
							url: `/pages/speakDetail/speakDetail?id=${item.collectedId}`
						})
					}
				} else {
					Toast.showToast('该产品已被删除')
				}
			}
		}
	}
