		...
		methods: {
			...
			init(mescroll) {
				...
				Toast.showLoading('加载中...', function() {
					let data = {
						url: '/app/service/user/upload/list',
						data: data5,
						success: function(res) {
							...
							if (res.data.success == true) {
								for (var j = 0; j < res.data.body.page.list.length; j++) {
									...
									item.year = item.uploadDate.substring(0, item.uploadDate.lastIndexOf('-'))
									item.date = item.uploadDate.substring(item.uploadDate.lastIndexOf('-') + 1, item.uploadDate.lastIndexOf(
										'-') + 3)
									item.time = item.uploadDate.substring(item.uploadDate.indexOf(' ') + 1, item.uploadDate.length)
									self.list.push(item)
								}
