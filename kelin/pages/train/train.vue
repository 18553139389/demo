	...
	export default {
		...
		methods: {
			...
			downloader(path) {
				...
				uni.navigateTo({
				    url: `/pages/parse/parse?url=${url}&type=1`
				})
			},
			onBackPress() {
				//监听back键，关闭弹出菜单
			...
			shareInfo() {
				let shareInfo = {
					href: `https://sale.clingcenter.net/static/share/productTrain.html?ids=${this.ids}&token=${this.message.Token}`,
