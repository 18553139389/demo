			 ...
			 @emptyclick="emptyClick" @topclick="topClick" style="width: 100%;">
				<ul>
					<li v-for="(v,k) in list" :key="k">
						<view class="top">
							<view style="width: 30%;">{{v.date}}日</view>
							<view style="width: 70%;">标题：{{v.title}}</view>
						</view>
						<view class="bottom" style="margin-top: 20upx;">
							<view style="width: 30%;font-size: 12px;color: #999;">{{v.year}}</view>
							<view style="width: 70%;font-size: 12px;color: #666;">{{v.feedbackContent}}</view>
						</view>
					</li>
				</ul>
			</mescroll-uni>
						...
						success: function(res) {
							...
							if (res.data.success == true) {
								for (var j = 0; j < res.data.body.page.list.length; j++) {
									...
									item.year = item.feedbackDate.substring(0, item.feedbackDate.lastIndexOf('-'))
									item.date = item.feedbackDate.substring(item.feedbackDate.lastIndexOf('-') + 1, item.feedbackDate.lastIndexOf(
										'-') + 3)
									item.time = item.feedbackDate.substring(item.feedbackDate.indexOf(' ') + 1, item.feedbackDate.length)
									self.list.push(item)
								}
