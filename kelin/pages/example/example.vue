<template>
	<view class="contain">
		<view class="fixed">
			<cu-custom :isBack="true" :Color="Color" :isIcon="true" bgColor="bg-shadeTop text-white" @goRight.native="goRight">
			...
			@emptyclick="emptyClick" @topclick="topClick">
				<ul>
					<li v-for="(v,k) in list" :key="k" @click.stop="goDetail(v.id)">
						<image :src="v.coverImage" mode="aspectFit"></image>
						<p>{{v.title}}</p>
					</li>
				</ul>
			</mescroll-uni>
		...
		methods: {
			...
			init(mescroll) {
				...
				Toast.showLoading('加载中...', function() {
					let data = {
						url: '/app/service/storyCase/list',
						data: data2,
						success: (res) => {
