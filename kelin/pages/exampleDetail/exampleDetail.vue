		...
		methods: {
			init() {
				...
				Toast.showLoading('加载中...', function() {
					let data = {
						url: '/app/service/storyCase/detail',
						data: data2,
						success: (res) => {
							console.log(res)
							if (res.data.success == true) {
								Toast.hideLoading()
								self.obj = res.data.body.storyCase
								self.store = res.data.body.storyCase.collected
								self.liked = res.data.body.storyCase.liked
								self.count = res.data.body.storyCase.likedCount
							} else {
								Toast.hideLoading()
			...
			downloader(path) {
				...
				uni.navigateTo({
				    url: `/pages/parse/parse?url=${url}&type=2`
				})
			},
			goZan() {
				let self = this
