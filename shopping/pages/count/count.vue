	...
	<view class="contain">
		<view class="fixed">
			<cu-custom :isBack="true" :Color="Color" :isIcon="true" bgColor="bg-shadeTop text-white" @goRight="goRight">
				...
				<block slot="content">订单报表</block>
				<block slot="right">打印</block>
			</cu-custom>
		</view>
		<view class="wrapper">
			<view class="tabs" @tap.stop="">
				<view class="tab-item" :class="itemIndex == k ? 'active' : ''" v-for="(v,k) in tab" :key="k" @tap="changeItem(k)">{{v.name}}</view>
			</view>
			<mescroll-uni :down="downOption" @down="downCallback" :up="upOption" @up="upCallback" @init="mescrollInit"
			@emptyclick="emptyClick" @topclick="topClick">
				<view class="lists">
					<view class="list-item">
						<view style="text-align: left;">订单日期</view>
						<view>订单数量</view>
						<view style="text-align: right;">订单金额</view>
					</view>
					<view class="list-item" v-for="(v,k) in list" :key="k">
						<view style="text-align: left;">{{v.date}}</view>
						<view>{{v.count}}</view>
						<view style="text-align: right;">￥{{v.amount}}</view>
					</view>
				</view>
			</mescroll-uni>
		</view>
...
<script>
	...
	import MescrollUni from "../../components/mescroll-unis/mescroll-uni.vue"
	export default {
		data() {
			return {
				Color: '#333',
		...
		methods: {
			...
			init(mescroll) {
				...
				let data2 = {
					...
					type: this.tab[this.itemIndex].id,
					pageNo: mescroll.num,
