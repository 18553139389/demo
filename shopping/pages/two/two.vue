<template>
	<view class="contain">
		...
		<view class="forms">
			<input type="text" v-model="call" placeholder="请输入原手机号" disabled>
			<view class="code">
			...
			<view class="change" @tap="goNext">下一步</view>
		</view>
	</view>
</template>
		...
		methods: {
			goNext() {
				...
				if (this.code !== this.validate) {
					Toast.showToast('验证码输入错误')
					return
				}
				uni.navigateTo({
					url: '/pages/three/three?code=' + this.code
				})
			},
			getValidate() {
				let self = this
